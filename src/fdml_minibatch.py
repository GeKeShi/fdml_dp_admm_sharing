...
class server(nodes):
	...
	def update(self, s_Q, index):
		...
		"""
		self.train_batch_features = self.train_label[index:index + self.train_size]
		self.s_Q = s_Q
		self.z_old = self.z[:]
...
class worker(nodes):
	...
	def update_no_noise(self, s_Q, z, y, index):
		self.train_batch_features = self.train_feature[index: index+self.train_size]
		self.s_Q = s_Q
		self.z = z
		self.y = y
...
class coord:
	...
	def train(self):
		...
		for t in range(self.config["max_iter"]):
			...
			for offset in range(0, self.server.data_size-self.server.train_size+1, self.server.train_size):
				if True == self.config["is_with_noise"]:
					for i in range(self.config["num_workers"]):
						# time_xi_start = time.time()
						self.worker[i].update_and_append_noise(s_Q, z, y, offset, method=self.config["noise_method"])
						# self.history["train_time_x"][t, i] = time.time() - time_xi_start
				else:
					for i in range(self.config["num_workers"]):
						# print("workers {} start in iter {}".format(i, t))
						# time_xi_start = time.time()
						self.worker[i].update_no_noise(s_Q, z, y, offset)
						# self.history["train_time_x"][t, i] = time.time() - time_xi_start
				...
				if (offset/self.server.train_size)%10 == 0:
